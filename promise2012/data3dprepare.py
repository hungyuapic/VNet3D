...
def prepare3dtraindata(srcpath, maskpath, trainImage, trainMask, number, height, width, shape=(16, 256, 256),
    ...
    for i in range(number):
        ...
        if imagearray.size == index * height * width and maskarray.size == index * height * width:
            imagearray = np.reshape(imagearray, (index, height, width))
            maskarray = np.reshape(maskarray, (index, height, width))
        else:
            print('Skip this file: ', number)
            continue
...
def preparenoduledetectiontraindata():
    ...
    width = 512
    number = 50
    srcpath = "C:\\Data\\PROMISE2012\\promise2012\\process\\image\\"
    maskpath = "C:\\Data\\PROMISE2012\\promise2012\\process\\mask\\"
    trainImage = "C:\\Data\\PROMISE2012\\promise2012\\segmentation\\image"
    trainMask = "C:\\Data\\PROMISE2012\\promise2012\\segmentation\\mask"
#    prepare3dtraindata(srcpath, maskpath, trainImage, trainMask, number, height, width, (16, 96, 96), 10, 16)
    prepare3dtraindata(srcpath, maskpath, trainImage, trainMask, number, height, width, (64, 128, 128), 10, 16)
